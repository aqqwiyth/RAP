...
function toBeanText(bean) {
    ...
    var packageName = "com.raycloud.rap.vo";
    if (packageName) {
 ...
 * @returns {{}}
 */
function getBeanFieldFromJson(text, parameterList) {
    ...
    bean = {name: className, val: bean, memo: memo};
 ...
 * @returns {string}
 */
function getTypeFromJsonVal(val, key, attrClassAry, memo) {
    var old = val;
    ...
    if (typeofStr === 'number') {
        ...
        if (isArray(val)) {
            ...
            for (var objKey in val) {
                var fieldValue = val[objKey];
                $.each(memo[key].parameterList, function (i, v) {
                    if (v.identifier == objKey) {
                        objMemo[objKey] = v;
                    }
                });
                bean[objKey] = getTypeFromJsonVal(fieldValue, objKey, attrClassAry, objMemo);
            }
            attrClassAry.push({name: typeName, val: bean, memo: objMemo});
            return typeName;
