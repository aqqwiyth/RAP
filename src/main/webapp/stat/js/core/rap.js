...
(function () {
    ...
    var ws = rap.workspace,       // workspace module
        ...
        TEMPLATE = {            // static template
            ...
            "REQUEST_BEGIN": "<h2>请求参数列表</h2><table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-identifier\">变量名</td><td class=\"head-name\">含义</td><td class=\"head-type\">类型</td><td class=\"head-type\">必传</td><td class=\"head-remark\">备注</td></tr>",
            "REQUEST_BEGIN_EDIT": "<h2>请求参数列表</h2><table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-op\">OP</td><td class=\"head-identifier\">变量名</td><td class=\"head-name\">含义</td><td class=\"head-type\">类型</td><td class=\"head-type\">必传</td><td class=\"head-remark\">备注</td></tr>",
            "REQUEST_END": "</table>",
            ...
            "RESPONSE_BEGIN": "<h2>响应参数列表</h2><table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-identifier\">变量名</td><td class=\"head-name\">含义</td><td class=\"head-type\">类型</td><td class=\"head-type\">必传</td><td class=\"head-remark\">备注</td></tr>",
            "RESPONSE_BEGIN_EDIT": "<h2>响应参数列表</h2><table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-op\">OP</td><td class=\"head-identifier\">变量名</td><td class=\"head-name\">含义</td><td class=\"head-type\">类型</td><td class=\"head-type\">必传</td><td class=\"head-remark\">备注</td></tr>",
            "RESPONSE_END": "</table>",
    ...
    ws.edit = function (id, key) {
        ...
        switch (key) {
            ...
            case "param-required":
                ...
                el = getTd(id, key);
                // oldValue = b.trim(el.innerHTML)
                oldValue = b.trim(param.required);
                str += getEditInputHtml(oldValue, width, CONFIG.REMARK_MAX_LENGTH);
                break;
            default:
     ...
     */
    ws.dataTypeSelectChanged = function (parameterId, value, param) {
        if (!param) param = "dataType";
        p.setParameter(parameterId, value, param);
    ...
    ws.showJSONToJava = function (actionId, type) {
        ...
        if (type != "request") {
            postData = 'type=' + type + '&actionData=' + encodeURIComponent(JSON.stringify(action));
            parameterList = action.responseParameterList;
        }
        b.ajax.post(URL.queryMockData, postData, function (xhr, response) {
            try {
                ...
                $.each(beans, function (i, v) {
                    ...
                    if (i == 1) {
                        $("#jsonToJava-container").append("<p class='more-class-tip'>包含对象关系的类.</p>")
                    }
                    var html = '<div><button class="right-button copy-button">copy code</button>' + '<h5>' + v.name + '</h5>' + '<textarea style="width:100%;height:150px;overflow-y:auto;"  class="result ' + textCss + '" >' + beanText + '</textarea></div>';
                    $("#jsonToJava-container").append(html);
                });
            } catch (e) {
                console.error(e);
                showMessage(CONST.ERROR, ELEMENT_ID.WORKSPACE_MESSAGE, MESSAGE.FATAL_ERROR);
    ...
    function getAInfoHtml(a) {
        var head = "<h2 style='margin-top:20px;'>接口详情 <span style='font-size: 14px; color: #999;'>(id: " + a.id
                ...
                + a.id + ");\">Mock数据</button>" +
                "&nbsp;&nbsp;<button class=\"btn btn-success btn-xs\" onclick=\"ws.showJSONToJava("
                + a.id + ",'request');\">生成请求Model</button>" +
                "&nbsp;&nbsp;<button class=\"btn btn-success btn-xs\" onclick=\"ws.showJSONToJava("
                + a.id + ",'response');\">生成响应Model</button>" +
    ...
    function getPTRHtml(param, level) {
        ...
        str += getDataTypeEditSelectHtml(param.id, param.dataType, "dataType");
        str += getRequiredEditSelectHtml(param.id, param.required, "required");
    ...
    function getRequiredEditSelectHtml(id, type, param) {
        var str = "",
            typeList = [false, true],
            typeListNum = typeList.length;
        str += "<td id='td-param-required-" + id + "' class='td-param required'>";
        if (_isEditMode) {
            str += "<select id='select-required-" + id + "' class='select-dataType' on" + CONFIG.KEYPRESS_EVENT_NAME + "='ws.dataTypeKeyPressed(event, " +
                id + ");' onchange='ws.dataTypeSelectChanged(" + id + ", this.value,\"required\");'>";
            for (var i = 0; i < typeListNum; i++) {
                var item = typeList[i];
                str += "<option value='" + item + "'" + (item == type ? " selected='true'" : "") + ">" + util.escaper.escapeInH(item) + "</option>";
            }
